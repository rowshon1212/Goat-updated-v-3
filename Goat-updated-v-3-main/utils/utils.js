...
const isGoogleDriveConfigured = !!(clientId && clientSecret && refreshToken);
if (!isGoogleDriveConfigured) {
	log.warn("CREDENTIALS", "Gmail/Google Drive credentials are not set in config.json > credentials.gmailAccount — Google Drive upload features will be disabled. The bot will continue to start normally.");
}
...
function _requireDriveApi() {
	if (!driveApi)
		throw new Error("Google Drive is not configured. Add a valid clientId, clientSecret, and refreshToken under config.json > credentials.gmailAccount to use this feature.");
	return driveApi;
}
const word = [
...
const drive = {
	...
	async uploadFile(fileName, mimeType, file) {
		...
		const driveApi = _requireDriveApi();
		let response;
		try {
