...
const builtinModulesSet = new Set([
	...builtinModules,
	...builtinModules.map(m => `node:${m}`)
]);
const exec = (cmd, options) => new Promise((resolve, reject) => {
	const child = require("child_process").exec(cmd, options, (err, stdout) => {
		if (err) return reject(err);
		resolve(stdout);
	});
	// Safety timeout: never let a stuck/slow install hang the whole bot boot forever.
	const killTimeout = setTimeout(() => {
		child.kill();
		reject(new Error(`Timed out after 60s installing via: ${cmd}`));
	}, 60000);
...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
	...
	for (const folderModules of folders) {
		...
		const fullPathModules = path.normalize(process.cwd() + `/modules/${folderModules}`);
			...
			try {
				...
				if (allPackage) {
					allPackage = allPackage.map(p => p.match(/[`'"]([^`'"]+)[`'"]/)[1])
						...
						.filter(p => !builtinModulesSet.has(p) && !builtinModulesSet.has(p.split("/")[0]));
