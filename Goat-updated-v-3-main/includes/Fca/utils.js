...
function generateClientHints(ua) {
    ...
    var chromeMatch = ua.match(/Chrom(?:e|ium)\/([0-9]+)/);
    var chromeVersion = chromeMatch ? chromeMatch[1] : null;

    var uaHint;
    if (chromeVersion) {
        uaHint = "\"Chromium\";v=\"" + chromeVersion + "\", \"Not;A=Brand\";v=\"24\", \"Google Chrome\";v=\"" + chromeVersion + "\"";
    } else {
        var safariMatch = ua.match(/Version\/([0-9]+)/);
        var safariVersion = safariMatch ? safariMatch[1] : "18";
        uaHint = "\"Safari\";v=\"" + safariVersion + "\", \"Not;A=Brand\";v=\"24\"";
    }
...
function getHeaders(url, options, ctx, customHeader) {
    ...
    var clientHints = generateClientHints(ua);
    ...
    var headers = {
        ...
        "sec-ch-ua": clientHints["sec-ch-ua"],
        "sec-ch-ua-mobile": clientHints["sec-ch-ua-mobile"],
        "sec-ch-ua-platform": clientHints["sec-ch-ua-platform"],
...
var _lastPostRequestTime = 0;
var _throttleMinGapMs = 300;
var _throttleDelayRange = [150, 400];

function handleThrottling() {
    var now = Date.now();
    var elapsed = _lastPostRequestTime ? now - _lastPostRequestTime : Infinity;
    _lastPostRequestTime = now;
    if (elapsed < _throttleMinGapMs) {
        var min = _throttleDelayRange[0];
        var max = _throttleDelayRange[1];
...
function post(url, jar, form, options, ctx, customHeader) {
    ...
    return handleThrottling().then(function () {
        return request(op);
    }).then(function (res) {
        return res;
    });
...
function formatDeltaMessage(m) {
    ...
    // mentionTypes: { userId: "p"|"s"|"t"|"a"|"c"|"cu"|"ai"|"n" }
    // Types from meta-main:
    //   p  = person (normal mention, sends notification)
    //   s  = silent (mention with no notification)
    //   t  = thread/@everyone (mention all members)
    //   a  = active/@here (mention active members only)
    //   c  = community channel
    //   cu = custom command
    //   ai = AI mention
    //   n  = unknown
    var mentionTypes = {};
    var hasMentionEveryone = false; // type "t"
    var hasMentionHere = false;     // type "a"
    ...
    if (mdata.length > 0) {
        for (var i = 0; i < mdata.length; i++) {
            ...
            var _t = mdata[i].t || "p"; // mention type, default "p" = person
            mentions[_id] = body.substring(_o, _o + _l);
            mentionTypes[_id] = _t;
            if (_t === "t") hasMentionEveryone = true;
            if (_t === "a") hasMentionHere = true;
        }
    } else if (
        ...
        for (var key in gbData) {
            ...
            if (entry && entry.asMap && entry.asMap.data) {
                ...
                var _gt = d.type && d.type.asString ? d.type.asString : "p";
                if (_gid != null) {
                    mentions[_gid] = body.substring(_go, _go + _gl);
                    mentionTypes[_gid] = _gt;
                    if (_gt === "t") hasMentionEveryone = true;
                    if (_gt === "a") hasMentionHere = true;
                }
            }
        }
    }
    ...
    return {
        ...
        mentions: mentions,
        mentionTypes: mentionTypes,             // { userId: "p"|"s"|"t"|"a"|"ai"|... }
        mentionedIDs: Object.keys(mentions),    // ["userId1", "userId2", ...] — easy array
        hasMentionEveryone: hasMentionEveryone, // true if @everyone (type "t")
        hasMentionHere: hasMentionHere,         // true if @here (type "a")
...
function saveCookies(jar) {
    return function (res) {
        ...
        cookies.forEach(function (c) {
            if (c.indexOf("facebook.com") > -1 || c.indexOf("messenger.com") > -1 || c.indexOf("Domain=") === -1) {
                try {
                    jar.setCookie(c, "https://www.facebook.com");
                } catch (_) {}
                try {
                    var messengerCookie = c
                        .replace(/domain=\.?facebook\.com/i, "domain=.messenger.com")
                        .replace(/domain=\.?messenger\.com/i, "domain=.messenger.com");
...
module.exports = {
    ...
    generateClientHints: generateClientHints,
    get: get,
    post: post,
