...
function buildAPI(globalOptions, html, jar) {
    ...
    var xsCookie = cookies.find(cookie => cookie.cookieString().startsWith("xs="));
    if (!userCookie && !tiktikCookie && !xsCookie) {
        return log.error("Error! Your cookiestate is not valid!");
    }
    var checkpointPatterns = ["/checkpoint/block/?next", "login_blocked", "account_locked"];
    if (checkpointPatterns.some(p => html.includes(p))) {
        log.warn("checkpoint", "Facebook returned a checkpoint/block warning for this appstate. Your account ID may be at risk — avoid frequent restarts, sending bulk messages, or logging in from new locations. Continuing login attempt, but if it fails, replace the appstate.");
    }
    userID = (tiktikCookie || userCookie || xsCookie).cookieString().split("=")[1];
    ...
    // ─── Auto-Cookie & fb_dtsg Background Refresh (anti-lock) ───────────────
    if (!global._fcaAutoRefreshIntervals) global._fcaAutoRefreshIntervals = {};
    var _autoRefreshKey = String(userID || clientID);
    if (global._fcaAutoRefreshIntervals[_autoRefreshKey]) {
        clearInterval(global._fcaAutoRefreshIntervals[_autoRefreshKey]);
        delete global._fcaAutoRefreshIntervals[_autoRefreshKey];
    }
    var _autoRefreshInterval = setInterval(function () {
        if (typeof api.refreshFb_dtsg === "function") {
            api.refreshFb_dtsg(function (err) {
                if (err) log.warn("autoRefresh", "Failed to auto-refresh fb_dtsg/cookies:", err && err.message ? err.message : err);
                else log.info("autoRefresh", "Auto-refreshed session cookie and fb_dtsg token.");
            });
        }
    }, 20 * 60 * 1000);
    if (typeof _autoRefreshInterval.unref === "function") _autoRefreshInterval.unref();
    global._fcaAutoRefreshIntervals[_autoRefreshKey] = _autoRefreshInterval;

    api.stopAutoRefresh = function () {
        if (global._fcaAutoRefreshIntervals[_autoRefreshKey]) {
            clearInterval(global._fcaAutoRefreshIntervals[_autoRefreshKey]);
            delete global._fcaAutoRefreshIntervals[_autoRefreshKey];
        }
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
    ...
    mainPromise
        .then(async () => {
            // ── rx-fca credit banner ──────────────────────────────────────────────
            var pkgVersion = '1.2.29';
            try { pkgVersion = require('./package.json').version; } catch (_) {}
            var C = {
                reset: '\x1b[0m', bold: '\x1b[1m', dim: '\x1b[2m',
                bGreen: '\x1b[92m', bCyan: '\x1b[96m', bMagenta: '\x1b[95m',
                bYellow: '\x1b[93m', bBlue: '\x1b[94m', bWhite: '\x1b[97m',
                bgMagenta: '\x1b[45m', white: '\x1b[37m',
            };
            ...
            console.log(
                '   ' + C.bCyan + '✦' + C.reset + '  ' +
                C.dim + 'E2EE + MQTT Bridge for Node.js' + C.reset
            );
            console.log(line);
            console.log(
...
function login(loginData, options, callback) {
    ...
    const autoUpdate = options && typeof options.autoUpdate !== 'undefined' ? options.autoUpdate : true;
    if (options && typeof options.autoUpdate !== 'undefined') {
        delete options.autoUpdate;
    }
    if (autoUpdate && !global.stfcaUpdateChecked) {
